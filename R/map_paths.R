...
map_paths <- function(input_dir, output_dir, files) {
  ...
  # Keep native separators for returned paths, but normalize a copy for matching.
  to_fs <- function(x) chartr("\\", "/", x)
  input_dir_fs <- to_fs(input_dir)
  files_fs <- to_fs(files)

  # Remove any trailing slashes on roots for consistent matching
  trim_trailing_slash <- function(x) sub("/+$", "", x)
  input_dir_fs <- trim_trailing_slash(input_dir_fs)
  ...
  prefix_re <- paste0("^", escape_regex(input_dir_fs), "(/|$)")
  ...
  out_paths <- unname(vapply(
    ...
    function(r) {
      if (!nzchar(r)) {
        ...
        do.call(
          file.path,
          c(list(output_dir), strsplit(r, "/", fixed = TRUE)[[1]])
        )
      }
    },
    character(1)
  ...
  out_dirs <- unique(dirname(out_paths))
